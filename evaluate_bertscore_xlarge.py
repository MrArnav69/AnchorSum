#!/usr/bin/env python3
"""
Simplified BERTScore Evaluation Script with DeBERTa-XLarge
Clean format: id, bertscore_precision, bertscore_recall, bertscore_f1
Uses microsoft/deberta-xlarge-mnli for better accuracy
"""
...
try:
    ...
    try:
        ...
        print("⚠️ BERTScore not available - install with: pip install bertscore")
        HAS_BERTSCORE = False
...
class SimpleBERTScoreEvaluator:
    def __init__(self, model_type='microsoft/deberta-xlarge-mnli', device='cuda:0', batch_size=8):
        """Initialize BERTScore evaluator with DeBERTa-XLarge"""
        self.device = device
    ...
    def calculate_scores(self, data: List[Dict], original_documents) -> pd.DataFrame:
        """Calculate BERTScore with proper ID matching"""
        results = []
        ...
        if HAS_BERTSCORE and len(summaries) > 0:
            ...
            try:
                # Calculate BERTScore (summary vs reference)
                P, R, F1 = bert_score(
                    summaries,
                    matched_references,
...
def main():
    ...
    print("🔬 BERTSCORE EVALUATION (DeBERTa-XLarge)")
    print("=" * 60)
    print(f"📋 Format: id, bertscore_precision, bertscore_recall, bertscore_f1, method")
    ...
    evaluator = SimpleBERTScoreEvaluator(
        ...
        device='cuda:0',
        batch_size=4  # Smaller batch for xlarge model
    )
    
    # Create output directory
    output_dir = 'bertscore_xlarge_results'
    os.makedirs(output_dir, exist_ok=True)
