#!/usr/bin/env python3
"""
BARTScore Only Evaluation Script
...
class BARTScoreOnlyEvaluator:
    ...
    def calculate_scores(self, data: List[Dict], original_documents) -> pd.DataFrame:
        ...
        logger.info("Calculating BARTScore sum2doc scores...")
        try:
            ...
            logger.info(f"✅ BARTScore sum2doc calculated successfully (first 3 scores: {sum2doc_scores[:3]})")
        except Exception as e:
            logger.error(f"Error calculating BARTScore sum2doc: {e}")
            sum2doc_scores = [0.0] * len(summaries)
        
        logger.info("Calculating BARTScore doc2sum scores...")
        try:
            ...
            logger.info(f"✅ BARTScore doc2sum calculated successfully (first 3 scores: {doc2sum_scores[:3]})")
        except Exception as e:
            logger.error(f"Error calculating BARTScore doc2sum: {e}")
...
def main():
    ...
    print("🔬 BARTScore ONLY EVALUATION")
    print("=" * 60)
    ...
    evaluator = BARTScoreOnlyEvaluator(device='cuda:0', batch_size=8)
    
    # Create output directory
    output_dir = 'bartscore_results'
    os.makedirs(output_dir, exist_ok=True)
    ...
    if all_results:
        ...
        for _, row in combined_summary.iterrows():
            ...
            print(f"   Truncated samples: {int(row.get('truncated_count', 0))}")
    
    print(f"\n📁 Results saved to: {output_dir}/")
    print("🎉 BARTScore evaluation completed!")
